...
class Boost_converter(gym.Env):
    """
    Boost converter model following gym interface.
    ...
    Example 1:
    env = Boost_converter(Vs = 200, L = 1, C = 1, R =  1, G = .1, Vdes = 400, dt = 1e-2)
    env.reset()
    env.plot()
    ...
    def __init__(self, Vs = 200, L = 0.001, C = 0.001, R = 0.001, G = 0.04, Vdes = 400, dt = 1e-5):
        super(Boost_converter, self).__init__()
    ...
    def equlibrium(self):
        ...
        if (sol2<=1) & (sol2>=0):
            ...
            raise ValueError("Non feasible equlibrium")
        return self.Vdes, self.G*self.Vdes/sol2, duty_ratio